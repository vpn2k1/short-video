...
export const guessStyle = (all: string, scenes: ScriptScene[]): StyleId => {
  ...
  if (/truyện ma|chuyện ma|ma ám|hồn ma|oan hồn|bóng ma|rùng rợn|kinh dị|chuyện lạ có thật|truyền thuyết đô thị|nhà hoang|ba giờ sáng|3 giờ sáng|3:00 sáng/i.test(all)) return "horror";
  if (/chúc tết|chúc mừng năm mới|năm mới|tết nguyên đán|ngày tết|đón tết|du xuân|lì xì|giao thừa|mùng\s*\d|an khang|vạn sự như ý|khuyến mãi tết|sale tết|ưu đãi tết/i.test(all)) return "festive";
  ...
  if (/dòng thời gian|cột mốc|từ năm \d{4}.{0,40}(đến nay|tới nay)|lịch sử (hình thành|phát triển)|qua các thời kỳ|tiểu sử/i.test(all)) return "timeline";
  if (/lịch trình|phượt|road ?trip|xuyên việt|food tour|bản đồ|cung đường|chặng đường|\d+\s*ngày\s*\d+\s*đêm|\d[\d.,]*\s*km\b/i.test(all)) return "map";
